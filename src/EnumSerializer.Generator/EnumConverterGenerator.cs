...
public class EnumConverterGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValueProvider<(
            Compilation,
            ImmutableArray<EnumDeclarationSyntax>
        )> compilationAndEnums = context.CompilationProvider.Combine(enumDeclarations.Collect());
...
)
{
    ...
    List<EnumInfo> enumsToProcess = GetTypesToGenerate(
        compilation,
        distinctEnums,
...
)
{
    ...
    foreach (var enumDeclarationSyntax in enums)
    {
        ...
        SemanticModel semanticModel = compilation.GetSemanticModel(
            enumDeclarationSyntax.SyntaxTree
        );
        if (
            semanticModel.GetDeclaredSymbol(enumDeclarationSyntax, cancellationToken: ct)
        ...
        foreach (var member in enumMembers)
        {
            ...
            members.Add(new(member.Name, displayName ?? ToSnakeCase(member.Name)));
        }

        enumsToProcess.Add(new(name: name, ns: nameSpace, members: members));
    }
    return enumsToProcess;
}
